...
def validate(args):
    ...
    gflops = get_gflops(model, img_size=args.img_size, device='cuda')
    mparams = get_mparams(model)
    _logger.info(f'Model {args.model} created, {mparams:.3f}M Params and {gflops:.3f}GFLOPs')
    ...
    results = OrderedDict(
        ...
        param_count=round(mparams, 2),
        flop_count=round(gflops, 2),
