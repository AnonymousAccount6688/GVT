...
class NeighborhoodAttention_1(nn.Module):
    def __init__(self, dim, num_heads, kernel_size, dilation, qkv_bias,
                 qk_scale, attn_drop, drop):
        super().__init__()
        self.dims = [dim // num_heads] * num_heads
    ...
    def forward(self, x):
        ...
        for i, xx in enumerate(xs):
            if i == 0:
                ...
                out = torch.cat([out, self.atts[i](xx)], dim=-1)
...
class NATLayer(nn.Module):
    def __init__(
        ...
        self.attn = NeighborhoodAttention_1(
            dim=dim,
            num_heads=num_heads,
            kernel_size=kernel_size,
            dilation=dilation,
