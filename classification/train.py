...
def main(args):
    ...
    if args.distributed:
        ...
        builtin_print('Training in distributed mode with multiple processes, 1 GPU per process. Process %d, total %d.'
    ...
    if args.apex_amp and has_apex:
        ...
        builtin_print("Neither APEX or native Torch AMP is available, using float32. "
    ...
    if args.local_rank == 0 and not args.torchscript:
        ...
        builtin_print(f'Model {safe_model_name(args.model)} created.')
        logging.info(f'{args.mparams:.3f}M Params and {args.gmacs:.3f} GFLOPs')
    ...
    print(f"{time.strftime('%m-%d %H:%M:%S')} start creating train dataset...")
    ...
    print(f"{time.strftime('%m-%d %H:%M:%S')} finished creating train dataset...")
    dataset_eval = None
    print(f"{time.strftime('%m-%d %H:%M:%S')} start creating val dataset...")
    if not args.disable_eval:
        dataset_eval = create_dataset(
            ...
            download=args.dataset_download,
            batch_size=args.batch_size)
    print(f"{time.strftime('%m-%d %H:%M:%S')} finished creating val dataset...")
    ...
    if args.rank == 0:
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(join(output_dir,
                                         f"{time.strftime('%m-%d %H:%M:%S')}"), mode='a'),
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            #         pdb.set_trace()
            #         print(f"batch_idx: {batch_idx}, Nan")
